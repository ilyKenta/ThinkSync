...
const ManageUsersPage = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_AZURE_API_URL}/api/admin/users`, {
          headers: {
  ...
  const handleSave = async (user: User) => {
    try {
      const token = localStorage.getItem('jwt');
      const res = await fetch(`${process.env.NEXT_PUBLIC_AZURE_API_URL}/api/admin/users/${user.id}/role`, {
        method: 'PUT',
        headers: {
